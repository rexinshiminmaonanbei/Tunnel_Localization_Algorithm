...
int Viterbi::calculate_prob(bool is_zero_t, const std::string& backtrack_txt) {
    if (is_zero_t) {
        ...
        std::vector<double> laststates_prob_copy(real_size, 0); 

        double maxReturn = 0;
        int returnIndex = 0;
        for (int i = 0; i < real_size; ++i) {
            double temp = laststates_prob[i] * initial_trans_prob[i] * obr_prob[i];
            laststates_prob_copy[i] = std::max(laststates_prob_copy[i], temp);
            if (maxReturn < laststates_prob_copy[i]) {
                maxReturn = laststates_prob_copy[i];
                returnIndex = startIndex + i;
            }
        }
        forward_match_results.push_back(returnIndex);

        // 归一化 laststates_prob_copy
        double sum = 0.0;
        for (int i = 0; i < real_size; ++i) {
            sum += laststates_prob_copy[i];
        }

        if (sum > 0) {  // 防止除以零
            for (int i = 0; i < real_size; ++i) {
                laststates_prob_copy[i] /= sum;  // 每个元素除以总和
            }
        }

        // 回溯用
        std::vector<BacktrackData> each_backtrack_data;
        each_backtrack_data.resize(real_size);
        for (int i = 0; i < real_size; ++i) {
            each_backtrack_data[i].probability = laststates_prob_copy[i];
            each_backtrack_data[i].last_state_index = startIndex + i;
        }
        writeBacktrackDataToFile(each_backtrack_data, backtrack_txt);
        each_backtrack_data.clear();

        // 更新 laststates_prob
        laststates_prob = laststates_prob_copy;
